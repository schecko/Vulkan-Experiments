...
//so this is a bit of a stupid workaround
namespace Windows
{
	void WindowsSleep(uint32_t sleepInMilli)
	{
		Sleep((DWORD)sleepInMilli);
	}
}
...
namespace Cy
{
	...
	LRESULT CALLBACK MessageHandler(HWND hwnd, UINT msg, WPARAM wP, LPARAM lP)
	{
		...
		case WM_MOUSEHOVER:
		{
			...
			input->mousePos = v2((float)GET_X_LPARAM(lP), (float)GET_Y_LPARAM(lP));
		}
		break;
		case WM_LBUTTONDOWN:
		{
	...
	WindowInfo NewWindowInfo(const char* appName, void* pointer, uint32_t clientWidth, uint32_t clientHeight)
	{
		...
		HWND windowHandle = CreateWindowA(appName,
			appName,
			WS_OVERLAPPEDWINDOW,
			CW_USEDEFAULT,
			CW_USEDEFAULT,
	...
	uint64_t GetAvgFps(const TimerInfo* timerInfo)
	{
		...
		{
			avgFps += (uint32_t)timerInfo->framesPerSec[i];
		}
		avgFps /= 10;
	...
	void Sleep(TimerInfo* timerInfo, int32_t desiredFps)
	{
		...
		int32_t frameTimeRemainingMilliSec = desiredFrameDurationMilliSec - SafeTruncate(timerInfo->frameTimeMilliSec);
		if (frameTimeRemainingMilliSec > 1 && frameTimeRemainingMilliSec < desiredFrameDurationMilliSec)
		{
			...
			Windows::WindowsSleep((uint32_t)sleepTimeMilliSec);
